    ...
    const generatePreview = (file) => {
        ...
        reader.onload = () => {
            if (file.type.startsWith('image/')) {
                setPreview(<img src={reader.result} alt="Preview" style={{ maxWidth: '100%', maxHeight: '400px', marginTop: '10px' }} />);
            } else if (file.type.startsWith('text/')) {
                setPreview(<pre style={{ whiteSpace: 'pre-wrap', maxHeight: '200px', overflow: 'auto', marginTop: '10px' }}>{reader.result}</pre>);
            } else if (file.type === 'application/pdf') {
                setPreview(<iframe src={reader.result} style={{ width: '100%', height: '500px', marginTop: '10px' }} title="PDF Preview"></iframe>);
            } else {
            ...
            {uploadFileNames.length > 0 && (
                <ListGroup style={{ marginTop: '10px', marginBottom: '10px' }}>
                    {uploadFileNames.map((file) => (
                        <ListGroup.Item style={{ fontSize: '14px', backgroundColor: '#f2f0ef' }} key={file}>
                            <div>
                                <CgAttachment />
