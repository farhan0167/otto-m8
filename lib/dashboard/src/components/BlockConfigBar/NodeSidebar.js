          ...
          {sideBarData && (
            <div>
              ...
              <hr/>
                ...
                {sideBarData.data.process_type === "hugging_face_model_card" && (
                    HuggingFaceModelCarSideBarData(sideBarData, onDataChange)
                )}

                {/* Ollama Server Generate Block Config */}
                {sideBarData.data.process_type === "ollama_server_generate" && (
                    OllamaServerGenerateSideBarData(sideBarData, onDataChange)
                )}

                {/* Ollama Server Generate Block Config */}
                {sideBarData.data.process_type === "ollama_server_chat" && (
                    OllamaServerChatSideBarData(sideBarData, onDataChange)
                )}

                {/* Integration Blocks go here */}
                {sideBarData.data.process_type === "http_post_request" && (
                    HTTPPostSideBarData(sideBarData, onDataChange)
                )} 
