...
const get_block_code = async (core_block_type) => {
  try {
      const response = await fetch(`http://localhost:8000/get_block_codes?core_block_type=${core_block_type}`);
...
export const AddBlockComponent = ({blocksData, handleSelectNode}) => {
  const handleOnNodeSelect = async (core_block_type) => {
    const block_code = await get_block_code(core_block_type)
    handleSelectNode(core_block_type, block_code)
  }
    return (
        ...
        <div key={vendorName} style={{marginBottom: '40px'}}>
          ...
          <ListGroup style={{maxHeight: '120px', overflowY: 'auto'}}>
            {Object.entries(Blocks).map(([blockName, core_block_type]) => (
                <ListGroup.Item key={core_block_type}  onClick={() => handleOnNodeSelect(core_block_type)}>{blockName}</ListGroup.Item>       
            ))}
          </ListGroup>
        </div>
      ))
