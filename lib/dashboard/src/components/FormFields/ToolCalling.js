...
//import { createRunConfigForNode } from '../../utils/workflow/CreateRunConfig';
...
export const ToolCalling = ({
    ...
    const [toolData, setToolData] = useState(blockData[field.name]);
    const [selectedTool, setSelectedTool] = useState(null);
    const [showToolConfig, setShowToolConfig] = useState(false);

    useEffect(() => {
        setToolData(blockData[field.name]);
    }, [blockData]);
    ...
    const handleDeleteTool = (toolIndex) => {
        const updatedTools = toolData.filter((_, index) => index !== toolIndex);
                ...
                <Formik
                    ...
                    onSubmit={(values, { setSubmitting }) => {
                        setTimeout(() => {
                            ...
                            setSubmitting(false);
                            handleClose();
                            reactFlowOnDataChange(field.name, updatedTools);
                        }, 400);
                    }}
                >
                    {({
...
const ToolConnectComponent = ({integrated_with, setFieldValue}) => {
     ...
     * 
     * @param {string} blockMetadata The integration block (core_block_type) type to select.
     */
    const handleSelectNode = async (blockMetadata) => {
        ...
        setConnectionData(block_data.data)
