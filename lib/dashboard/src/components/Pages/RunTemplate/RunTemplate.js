...
import { useInputHandlerHooks, useLoadingStateHook } from '../../InputHandles/hooks';
...
const RunTemplate = () => {
  ...
  /**
   * Periodically checks the health of the deployment until it is ready.
   * Essentially here so we know the server is ready to be queried. This
   * is needed since when loading HF models, given that its a blocking process,
   * means that the server won't be ready to respond.
   * @param {string} deployment_url - The URL of the deployment to check.
   * @returns {Promise<void>}
   */
  const checkHealth = async (deployment_url) => {
    setLoading({loading: true, message: 'Loading Resources...'});
    while(true){
      const health_check = await healthCheck(deployment_url);
      if (health_check) {
        setLoading({loading: false, message: ''});
        break
      }
        ...
        if (!deploymentUrl) {
          ...
          fetch(`http://localhost:8000/templates/${template_id}`, {
            ...
            .then(data => {
                ...
                setLoading({loading: false, message: ''});
                checkHealth(data.deployment_url);
            })
            .catch(err => {
              setError(err.message);
              setLoading({loading: false, message: ''});
  ...
  return (
    ...
    <Container fluid>
        ...
        </Row>): <p style={{height:'100vh'}}>{loading.message}</p>}
    </Container>
    </>
  )
}
