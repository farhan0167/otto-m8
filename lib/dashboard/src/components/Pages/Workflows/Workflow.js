...
export default function Flow() {
  ...
  useEffect(() => {
    const workload = location.state
    // If a new workflow is being created
    if (['new', 'preset'].includes(workload.type)) {
          const createDraftWorkflow = async (template) => {
            try {
              ...
              if (response.ok) {
                ...
                navigate('/workflow', {state: {type: 'draft', template_id: data.template_id, template: template}, replace: true})
          ...
          createDraftWorkflow(workload.template);
    }
    else if (workload.type === 'draft') {
        // Query draft table to get template id, then set nodes and everything
        console.log("Draft Workflow", workload)
        setDraftTemplateId(workload.template_id)
    }
    else if (workload.type === 'edit') {
    ...
    else if (workload.type === 'import') {
        // if an import, then we need a function that takes a backend template and converts it to a frontend template.
    }

    setNodes(workload.template.nodes)
    setEdges(workload.template.edges)
    setWorkflowMetadata({
      name: workload.template.name,
      description: workload.template.description
    })
            ...
            <Container fluid>
              <Row>
                ...
                <Col className='workflow-button-container'>
                  <Button id='add-block-button' onClick={addNodeButtonClick}>Add Block</Button>
