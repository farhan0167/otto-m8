...
export default function Flow() {
  ...
  useEffect(() => {
    ...
    // If a new workflow is being created
    if (['new', 'preset'].includes(draft_workflow.type)) {
        ...
        console.log("Draft Workflow", draft_workflow)
        setDraftTemplateId(draft_workflow.template.id)
    }
    else if (draft_workflow.type === 'edit') {
        // if edit, then dont really create a draft until the user saves the workflow.
        // have a state that tracks if a workflow deployed is an edit or not.
        // if edit, then gotta call an ednpoint that deletes the deployed workflow and then launches the new one.
    }
    else if (draft_workflow.type === 'import') {
        // if an import, then we need a function that takes a backend template and converts it to a frontend template.
    }
  ...
  const handleSelectNode = (core_block_type) => {
    ...
    if (core_block_type === 'output') {
      setNodes((nds) => {
        ...
        return [
          ...nds,
          initialDataOutputBlock({nodeType:'output', 'core_block_type': core_block_type}),
        ];
      });
    }
      ...
      setNodes((nds) => {
        return [
          ...
          initialDataInputBlock({nodeType:'input', 'core_block_type': core_block_type}),
        ];
      });
    }
    else {
  ...
  const handleDraftSave = () => {
      ...
      const updateDraftTemplate = async () => {
        ...
        console.log("Handle Save: template", template);
        try {
          ...
          console.log('Draft template updated:', data);
        } catch (error) {
      ...
      if (draftType === 'edit' && draftTemplateId === null) {
        console.log('Draft template ID is null');
          ...
          <div style={{ height: '100vh' }}>
            ...
            <Container fluid>
                <Box sx={{ display: 'flex', justifyContent: 'flex-end', marginTop: '10px', marginBottom: '20px' }}>
                  <Stack variant="outlined" aria-label="Loading button group" size='large' direction="row" spacing={1}>
                      <Button id='add-block-button'sx={{backgroundColor:'black', color:'white', width: '150px'}} onClick={addNodeButtonClick} size='large'>Add Block</Button>
                      <Button sx={{backgroundColor:'black', color:'white', width: '150px'}} onClick={handleRunWorkflowButton} size='large'>Deploy</Button>
                      <Button variant='contained' size='large' onClick={handleDraftSave}>Save</Button>
                  </Stack>
                </Box>
                <Box>
                  <div>
