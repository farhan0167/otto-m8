...
export const processNodeDataForBackend = (nodes, edges) => {
    ...
    nodes.forEach(node => {
      ...
      // Format block data to adhere to the backend's expected format(engine/block.py)
      const formattedNode = {
        name: formattedLabel,
      ...
      if (type === 'input') {
          ...
          if (data.core_block_type){
            ...
            process_metadata['files_to_accept'] = data.files_to_accept
            process_metadata['button_text'] = data.button_text
