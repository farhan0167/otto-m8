...
const Home = () => {
    ...
    const createNewWorkflow = async () => {
        ...
        const getInitialDataTextInput = async () => {
          ...
          let initialDataOutput = await getInitialDataOutput();
          initialDataTextInput = {...initialDataTextInput, ...nodeDefaults};
          initialDataOutput = {...initialDataOutput, ...nodeDefaults};
        const template = NewTemplate();
        template.nodes.push(initialDataTextInput);
        template.nodes.push(initialDataOutput);
        ...
        <Container fluid maxWidth='xl'>
            <Box sx={{ display: 'flex', justifyContent: 'flex-end', marginTop: '10px', marginBottom: '20px' }}>
                <Stack 
                    ...
                    xs={12}
                >
                    <Button color='primary' variant='contained' size='medium'>Import</Button>
                    <Button 
                        color='primary' 
                        variant='contained' 
                        sx={{width: '160px'}} 
                        size='medium' 
                        onClick={() => createNewWorkflow()}>New Workflow</Button>
                </Stack>
            </Box>
                ...
                <div style={{ marginTop: '50px' }}>
                    ...
                    <Table sx={{ minWidth: 650 }} aria-label="simple table" size="small">
                        ...
                        {draftWorkflows.map((row) => (
                            ...
                            hover
                            >
                            <TableCell sx={{ cursor: 'pointer' }} component="th" scope="row" onClick={() => openDraftWorkflow(row)}>
                                {row.name}
                            </TableCell>
